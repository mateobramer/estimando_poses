...
FIX respecto al original (entrenar_modelo_b.py):
    El train_transform original incluia RandomResizedCrop y RandomRotation,
    que mueven/recortan/rotan la IMAGEN. Pero los heatmaps se generaban con
    las coordenadas del CSV, que describen la posicion en la imagen ORIGINAL
    (sin recortar ni rotar). Resultado: la imagen que ve la red y el heatmap
    objetivo quedaban desalineados una fraccion del tiempo, lo cual es señal
    de entrenamiento contradictoria.

    Solucion aplicada aca: se sacan las transforms geometricas (crop, rotacion)
    del augmentation de train. Se mantienen las que NO mueven geometria
    (ColorJitter, GaussianBlur, RandomGrayscale) porque esas siguen ayudando
    contra overfitting sin desalinear nada. El tamaño final pasa a fijarse
    con un Resize directo a 224x224, igual que en val_transform.

USO:
    python entrenar_modelo_b_fix.py --epochs 3 --batch 8 --max_samples 500
    python entrenar_modelo_b_fix.py --epochs 50 --batch 32
...
def entrenar(args):
    ...
    train_transform = transforms.Compose([
        # Resize directo a 224x224 (no RandomResizedCrop): mismo tamaño que
        # ve val_transform, sin recortar geometria de forma aleatoria.
        transforms.Resize((224, 224)),
        # Estas 3 NO mueven pixeles de lugar, solo cambian color/nitidez —
        # las coordenadas del CSV siguen siendo validas despues de aplicarlas.
        transforms.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.2),
        transforms.GaussianBlur(kernel_size=3, sigma=(0.1, 2.0)),
        transforms.RandomGrayscale(p=0.1),
        # RandomResizedCrop y RandomRotation se sacaron de aca: requerian
        # transformar tambien las coordenadas (x,y) de cada keypoint del CSV
        # para no desalinear imagen vs heatmap objetivo. Si en el futuro se
        # quiere reincorporar augmentation geometrica, hay que aplicar la
        # misma transformacion afin a los keypoints, no solo a la imagen.
        transforms.ToTensor(),
...
def main():
    ...
    parser.add_argument("--sigma",         type=float, default=2.0,
                         help="Desvio estandar del heatmap gaussiano. Mas chico = mas exigente "
                              "(blob concentrado, dificil de entrenar). Mas grande = mas tolerante "
                              "(blob ancho, converge mas facil pero pico final menos preciso).")
    parser.add_argument("--max_samples",   type=int,   default=None)
    parser.add_argument("--salida_mejor",  default=None)
    parser.add_argument("--salida_ultimo", default=None)
    args = parser.parse_args()

    # nombres de salida: si no se pasaron, incluyen el sigma para no pisarse
    # entre corridas con distinto sigma
    if args.salida_mejor is None:
        args.salida_mejor = f"modelo_b_fix_sigma{args.sigma}_mejor.pth"
    if args.salida_ultimo is None:
        args.salida_ultimo = f"modelo_b_fix_sigma{args.sigma}_ultimo.pth"
